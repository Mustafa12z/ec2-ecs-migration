...
- **EC2 Instance**: Ubuntu 22.04 LTS (Jammy)
...
1. **Containerization**: Dockerfiles for the Flask API and Nginx frontend (sidecar pattern, one ECS task)
2. **ECS Infrastructure**: Terraform modules for VPC, IAM, RDS, ALB, ECS, CodeDeploy, Route53, CloudWatch
3. **CI/CD Pipeline**: GitHub Actions with OIDC, Trivy image/IaC scanning, SBOM + cosign signing
4. **Observability**: CloudWatch logs, metrics, and alarms (CPU/memory, ALB 5xx)
5. **Cutover**: Blue/green deployments via CodeDeploy, zero downtime

See [`MIGRATION.md`](MIGRATION.md) for the full record, cost breakdown, and
trade-offs behind each of these decisions.
