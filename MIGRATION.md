...
| GitHub OIDC provider + role bootstrapped via AWS CLI, not Terraform | `module.iam` originally modeled these, but it transitively depends on `module.rds` (for the execution role's secret-read policy) — applying just the IAM piece would have forced a full RDS/VPC/ALB provisioning cost just to get CI working before the rest of the stack existed | This one role + provider live outside Terraform state. If you want Terraform to own them later, re-add the resources to `modules/iam` and `terraform import` the existing AWS objects rather than re-creating them |
